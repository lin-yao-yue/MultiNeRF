...
# : 后面是参数的类型注释(字符串是解释说明，类型表达式是参数的类型)。 = 后是默认值。 -> 后是返回值类型
def linear_to_srgb(linear: _Array,  # Union类型，可以是 np.array, 也可以是jnp.array
...
def downsample(img, factor):
    ...
    img = img.reshape((sh[0] // factor, factor, sh[1] // factor, factor) + sh[2:])  # 元组之间的拼接
    img = img.mean((1, 3))  # (h//f, w//f, c)
    return img
