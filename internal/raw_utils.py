...
def pixels_to_bayer_mask(pix_x: np.ndarray, pix_y: np.ndarray) -> np.ndarray:
    ...
    '''
    pix_x: [num_patches, _patch_size, _patch_size]
    pix_y: [num_patches, _patch_size, _patch_size]
    '''
    # 返回bool array，*看作与运算，+看作或运算
    # Red is top left (0, 0).
    ...
    return np.stack([r, g, b], -1).astype(np.float32)  # [num_patches, _patch_size, _patch_size, 3]
