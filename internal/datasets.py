...
sys.path.insert(0, 'internal/pycolmap')  # 向module的搜索路径 添加 高优先级(0)搜索 的 module路径
sys.path.insert(0, 'internal/pycolmap/pycolmap')
import pycolmap  # 添加的module名


def load_dataset(split, train_dir, config):
    """
    Loads a split of a dataset using the data_loader specified by `config`.
    config.dataset_loader：The type of dataset loader to use.
    """
    # 不同类型的数据集的加载方式
    dataset_dict = {
        'blender': Blender,
...
class Dataset(threading.Thread, metaclass=abc.ABCMeta):  # 继承threading.Thread创建线程，需要重写init和run方法
    """Dataset Base Class.
  ...
  Attributes:
    ...
    camtoworlds: np.ndarray, a list of extrinsic camera pose matrices. 外参数矩阵
  ...
  """
    ...
    def __init__(self,
                 split: str,  # 划分 train，test
                 data_dir: str,
                 config: configs.Config):
        super().__init__()  # 继承并重写thread的init方法

        # Initialize attributes
        self._queue = queue.Queue(3)  # Set prefetch buffer to 3 batches. FIFO队列，3是队列上限
        self.daemon = True  # Sets parent Thread to be a daemon.
        self._patch_size = np.maximum(config.patch_size, 1)  # Resolution of patches sampled for training batches.
        # jax.process_count() 后端 jax 进程数量
        # 一个batch所包含的rays数量
        ...
        self.disp_images = None  # [n, h, w]
        self.normal_images = None  # [n, h, w, 3]
        self.alphas = None  # [n, h, w]
        self.poses = None
        ...
        self.images: np.ndarray = None  # [n, h, w, 3(RGB)]
        self.camtoworlds: np.ndarray = None  # [n, 4, 4]
        self.pixtocams: np.ndarray = None  # [3, 3]
        self.height: int = None  # int value h
        self.width: int = None  # int value w
        ...
        if self.render_path:
            ...
            if config.render_focal is not None:  # 如果该判断成立，则focal将是[n, float_value], 不再是单一的值，
                self.focal = config.render_focal  # 此时pixtocams也将变成 [n, 3, 3]
            if config.render_camtype is not None:
                if config.render_camtype == 'pano':
                    self._render_spherical = True
                else:
                    # 通过value访问enum，返回 ProjectionType.PERSPECTIVE
        ...
        self._n_examples = self.camtoworlds.shape[0]  # n
        ...
        if self.split == utils.DataSplit.TRAIN:
            self._next_fn = self._next_train  # utils.Batch
        else:
    ...
    """
        x = self._queue.get()  # utils.Batch
        if self.split == utils.DataSplit.TRAIN:
            return utils.shard(x)  # 通过 device_count 对 num_patches 进行分割 --> 并行训练
        else:
    ...
    def run(self):  # 把要执行的代码写到run函数里面 线程在创建后会直接运行run函数
        while True:  # 只要queue不满，就一直将utils.Batch添加到queue中
            self._queue.put(self._next_fn())
    ...
    @abc.abstractmethod
    def _load_renderings(self, config):  # 加载图片信息，向下继承，由不同加载类型的子类实现
    ...
    def _make_ray_batch(self,
                        pix_x_int: np.ndarray,  # [num_patches, _patch_size, _patch_size] for _next_train
                                                # or  [h,  w] for _next_test
                        pix_y_int: np.ndarray,  # [num_patches, _patch_size, _patch_size] for _next_train
                                                # or  [h,  w] for _next_test
                        cam_idx: Union[np.ndarray, np.int32],  # (num_patches, 1, 1)
                                                               # or single value for_next_test
                        lossmult: Optional[np.ndarray] = None  # [num_patches, _patch_size, _patch_size, 3] bool or None
                        ) -> utils.Batch:
    ...
    """
        ...
        if not self.render_path:
            batch['rgb'] = self.images[cam_idx, pix_y_int, pix_x_int]  # [num_patches, _patch_size, _patch_size, 3]
        if self._load_disps:
    ...
    def _next_train(self) -> utils.Batch:
        ...
        # 之后有将数组当作index使用的情况，而index数组必须是int类型，所以用randint进行初始化
        # 像素值(ray)采样 [num_patches, 1, 1]
        ...
        patch_dx_int, patch_dy_int = camera_utils.pixel_coordinates(
            self._patch_size, self._patch_size)  # [_patch_size, _patch_size]
        pix_x_int = pix_x_int + patch_dx_int  # broadcast to [num_patches, _patch_size, _patch_size]
        pix_y_int = pix_y_int + patch_dy_int  # broadcast to [num_patches, _patch_size, _patch_size]
        # Random camera indices.
        if self._batching == utils.BatchingMethod.ALL_IMAGES:
            # 从 n 张 pic 中随机选择 num_patches 张
            cam_idx = np.random.randint(0, self._n_examples, (num_patches, 1, 1))
        else:
            cam_idx = np.random.randint(0, self._n_examples, (1,))

        if self._apply_bayer_mask:  # 使用bayer_mask对raw_pic进行处理前先生成bayer_mask
            # Compute the Bayer mosaic mask for each pixel in the batch.
            # [num_patches, _patch_size, _patch_size, 3] bool
            lossmult = raw_utils.pixels_to_bayer_mask(pix_x_int, pix_y_int)
        else:  # 非raw_pic, 而是rgb_pic
            lossmult = None

        return self._make_ray_batch(pix_x_int, pix_y_int, cam_idx, lossmult=lossmult)  # utils.Batch
    ...
    def generate_ray_batch(self, cam_idx: int) -> utils.Batch:
        ...
        if self._render_spherical:
            camtoworld = self.camtoworlds[cam_idx]  # [4, 4]
            ...
            pix_x_int, pix_y_int = camera_utils.pixel_coordinates(  # [h, w]
    ...
    def _load_renderings(self, config):
        """Load images and poses from disk.
            ...
            In inherited classes, this method must set the following public attributes:
              ...
              camtoworlds: [N, 3, 4] array of extrinsic pose matrices.
              poses: [..., 3, 4] array of auxiliary pose data (optional).
              pixtocams: [3, 3] or [n, 3, 3] array of inverse intrinsic matrices.
              distortion_params: dict, camera lens distortion model parameters.
              height: int, height of images.
              width: int, width of images.
              focal: float, focal length to use for ideal pinhole rendering.
        ...
        if config.render_path:
            raise ValueError('render_path cannot be used for the blender dataset.')  # raise语句显式引发异常
        # camera相关信息
        pose_file = path.join(self.data_dir, f'transforms_{self.split.value}.json')  # f'{a}' 等价于 '{}'.format(a)
        with utils.open_file(pose_file, 'r') as fp:
            # pose字典
            meta = json.load(fp)
        images = []  # [n, h, w, 4(RGBA)]
        disp_images = []  # disparity MES [n,h,w]
        normal_images = []  # normals
        cams = []  # 外参数矩阵 [n, 4, 4]
        # 对于emulate，_ 顺序号
        for _, frame in enumerate(meta['frames']):  # frame: 每一张图片的路径，camera，外参数矩阵
            fprefix = os.path.join(self.data_dir, frame['file_path'])  # 图片路径

            # 加载图片并决定是否下采样
            def get_img(f, fprefix=fprefix):
                image = utils.load_img(fprefix + f)  # np.array (h,w,c) or np.array (h,w)
                if config.factor > 1:  # 成立则下采样，即是否减小图像分辨率来减轻训练压力
                    image = lib_image.downsample(image, config.factor)  # (h//f, w//f, c) or (h//f, w//f)
                return image

            '''
            TIFF 文件内部结构可以分成三个部分，分别为：文件头信息区（IFH）、图像文件目录（IFD）和 图像数据区
            '''
            if self._use_tiffs:  # 是否使用 32 bit TIFFS
                channels = [get_img(f'_{ch}.tiff') for ch in ['R', 'G', 'B', 'A']]
                # Convert image to sRGB color space.
                image = lib_image.linear_to_srgb(np.stack(channels, axis=-1))  # [h, w, c, 4] ->
            else:
                image = get_img('.png') / 255.  # [h,w,c] 数据在0-1之间

            images.append(image)  # [n, h, w, c]

            if self._load_disps:  # disparity MSE
                disp_image = get_img('_disp.tiff')  # [h, w]
                disp_images.append(disp_image)

            if self._load_normals:  # normal
                normal_image = get_img('_normal.png')[..., :3] * 2. / 255. - 1.  # [h, w, 3]
                normal_images.append(normal_image)

            # 外参数矩阵
            cams.append(np.array(frame['transform_matrix'], dtype=np.float32))

        self.images = np.stack(images, axis=0)  # [n, h, w, 4(RGBA)]
        if self._load_disps:
            self.disp_images = np.stack(disp_images, axis=0)  # [n, h, w]
        if self._load_normals:
            self.normal_images = np.stack(normal_images, axis=0)  # [n, h, w, 3]
            self.alphas = self.images[..., -1]  # [n, h, w]

        rgb, alpha = self.images[..., :3], self.images[..., -1:]  # [n, h, w, 3(RGB)] [n, h, w, 1(A)]
        self.images = rgb * alpha + (1. - alpha)  # Use a white background. [n, h, w, 3(RGB)]
        self.height, self.width = self.images.shape[1:3]  # h, w
        # 外参数矩阵
        self.camtoworlds = np.stack(cams, axis=0)  # [n, 4, 4]
        # 焦距，一个jason中的图片信息都是在同一个camera下拍摄得到的，所以对应着同一个focal
        self.focal = .5 * self.width / np.tan(.5 * float(meta['camera_angle_x']))  # float value
        # 内参数矩阵的逆
        self.pixtocams = camera_utils.get_pixtocam(self.focal, self.width,
                                                   self.height)  # [3, 3]
